...
public class gmlogin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			String qur="select * from gmregister where email=? and password=?";
			PreparedStatement ps1 = database.getconnection().prepareStatement(qur);
			ps1.setString(1,e);
			ps1.setString(2, p);
			ResultSet rs= ps1.executeQuery();
			if(rs.next()) {
				out.print("<html><body><script>alert('Group Manager login Successfully')</script></body></html>");
				...
				RequestDispatcher rd = request.getRequestDispatcher("gmlogin.html");
				rd.include(request, response);
			}
		}catch(Exception E) {
